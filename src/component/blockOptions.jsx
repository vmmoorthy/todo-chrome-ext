...
import { deleteData, insert } from "storage_engine";
...
const BlockOptions = ({ item }) => {
    ...
    const removePriority = uuid => setPriorityList(p => p.filter(p => p.uuid !== uuid))
    ...
    const togglePriority = (e) => e.target === e.currentTarget && setPriorityDropDown(p => !p)

    return (
        <div contentEditable={false} ref={r => scVRef.current = r} style={priorityDropDown ? {
            visibility: 'visible',
            opacity: 1,
        } : {}} className="blockOptions opacity-20 transition-opacity hover:opacity-100 grid grid-flow-col items-end absolute z-10  top-[-1rem] w-full ">
            ...
            <div tabIndex={0} onClick={togglePriority} style={priorityDropDown ? {
                visibility: 'visible',
                opacity: 1,
            } : {}} className="priority border-white border-solid border-[1px] cursor-pointer hover:bg-[#6D385A] priority grid grid-flow-col items-center bg-[#6A1B4D] gap-1 justify-center w-min h-min max-h-6 px-1 py-[0.2rem] rounded ">
                <div onClick={togglePriority} style={{ backgroundColor: priorityList.find(v => v.uuid === item.priority)?.color || "#fff5" }} className="color w-3 h-3 rounded-full "></div>
                <span onClick={togglePriority} className="text-[0.87rem] leading-none ">{priorityList.find(v => v.uuid === item.priority)?.text || "--"}</span>
                <svg onClick={togglePriority} className="w-4 h-4" width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path onClick={togglePriority} d="M32.8282 11.7188H7.17195C6.40242 11.7188 5.97273 12.5313 6.44929 13.0859L19.2774 27.9609C19.6446 28.3867 20.3516 28.3867 20.7227 27.9609L33.5509 13.0859C34.0274 12.5313 33.5977 11.7188 32.8282 11.7188Z" fill="white" />
                </svg>
                <div tabIndex={1} style={priorityDropDown ? {
                    visibility: 'visible',
                    opacity: 1,
                } : {}} className="dropDown bg-[#6A1B4D] border-white border-solid border-[1px] rounded p-1 absolute top-[1.5rem] ">
                    <div className="relative">
                        ...
                        {priorityList.map(v =>
                            <div key={v.uuid} className="Midium flex flex-row items-center justify-between hover:bg-[#6D385A]">
                                <span className="flex flex-row w-full justify-center items-center gap-1" onClick={e => updatePriority(e, v.uuid)}>
                                    <div className="color w-3 h-3 rounded-full " style={{ backgroundColor: v.color }}></div>
                                    <span className="w-full " suppressContentEditableWarning={true} contentEditable={edit}>{v.text}</span>
                                </span>
                                {edit && <div onClick={() => removePriority(v.uuid)} className="remove w-[0.85rem] h-[0.85rem] p-[0.05rem]">
            ...
            <div className="drag select-text border-white border-solid border-[1px] cursor-move hover:bg-[#6D385A] time text-[1rem] w-min h-min max-w-[8.5rem] text-center max-h-6 whitespace-nowrap px-1 py-0 leading-5 rounded bg-[#6A1B4D] "> : : : </div>
            {/* time */}
